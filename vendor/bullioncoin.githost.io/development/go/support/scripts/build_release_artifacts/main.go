...
func main() {
	...
	pkg := packageName(bin)
	repo := "bullioncoin.githost.io/development/go"
...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X bullioncoin.githost.io/development/go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X bullioncoin.githost.io/development/go/support/app.version=%s", version)
